...
🔴 **شرطان قبل `production`، وكلاهما يلزمه تثبيتُ `vc34` من `internal` على هاتفٍ حقيقيّ:**
① **الضوابط المعاكسة:** `/pricing` و`/portal` و`/<مدرسةٍ-أخرى>` ⇒ **المتصفّح لا التطبيق**.
   🔴 وهذا هو الضابط — لا فتحُ `/abdaawatmuaz` بنجاح. ضابطٌ لا يتغيّر مخرَجُه بين الصحيح
   والخطأ ليس ضابطاً.
② **إشعارٌ فعليٌّ يصل جهازاً** بعد نشر خبر. `FCM_PROJECT_ID` ضُبطت في مشروع `teacher`
   يوم `2026-09-05T01:19:23Z`، **وحالتُها لا تُقاس من خارج مشروع GAS إطلاقاً**.
...
والعزلُ يكلّف **ثلاثَ رفعاتٍ = ثلاثةَ أرقامٍ محروقة**. بند `play-upload-silently-dropped`
يبقى مفتوحاً.
...
آخر تحديث: 2026-09-05 (‏`vc34`/`3.1` حيٌّ على `internal` بتقييد App Links · استنساخُ سقوط
رفع Play 3/3 وعلاجٌ عاملٌ بجذرٍ **غير معزول** · إصلاحُ فحصِ ما قبل الرفع الذي كان يردّ 404 صامتاً)
